...
public class OpenTableController implements Initializable {
    ...
    void setReporte(ArrayList<ReporteMesa> apMesasDiario, oasisrimbd inter) throws RemoteException {
        ReporteMesa rp = null;
        for (ReporteMesa r : apMesasDiario) {
            if (r.getContrato().equals(client.getContrato())) {
                rp = r;
            }
        }
        ninvitados.setText(rp.getInvitados());
        ArrayList<Invitado> inv = inter.getInvitados();
        inv.removeIf((Invitado r) -> !r.getContrato().equals(client.getContrato()));
        inv.removeIf((Invitado r) -> !r.getFecha().equals(LocalDate.now().format(DateTimeFormatter.ISO_LOCAL_DATE)));
        for (Invitado i : inv) {
